...
defmodule CalendarTypeTests do
  use ExUnit.Case

  test "should return day of the week for provided date" do
    assert :calendar.day_of_the_week({2016, 7, 14}) == 4
  end
end

defmodule PossibleDatesTests do
  use ExUnit.Case

  test "verify schedule day ranges" do
    assert Meetup.possible_dates(2016, 2, :first) == 1..7
    assert Meetup.possible_dates(2016, 2, :second) == 8..14
    assert Meetup.possible_dates(2016, 2, :third) == 15..21
    assert Meetup.possible_dates(2016, 2, :fourth) == 22..28
    assert Meetup.possible_dates(2016, 2, :teenth) == 13..19
    assert Meetup.possible_dates(2016, 2, :last) == 23..29
  end
end

defmodule IsCorrectWeekdayTests do
  use ExUnit.Case

  test "verify is correct weekdays" do
    assert Meetup.is_correct_weekday?({2016, 7, 15}, :friday)
    refute Meetup.is_correct_weekday?({2016, 7, 14}, :friday)
  end
end
