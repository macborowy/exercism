...
defmodule EqualTests do

  use ExUnit.Case

  test "same single element lists are equal" do
    assert Sublist.compare([1], [1]) == :equal
  end

    # @tag :pending
  test "different single element lists are unequal" do
    assert Sublist.compare([1], [2]) == :unequal
  end

    # @tag :pending
  test "lists with same length but different element are unequal" do
    assert Sublist.compare([1, 2, 3], [4, 5, 6]) == :unequal
  end

  test "list with different types are unequal" do
    assert Sublist.compare([:atom], [1]) == :unequal
  end
end

defmodule SublistTests do

  use ExUnit.Case

    # @tag :pending
  test "single element sublist" do
    assert Sublist.compare([1], [1, 2]) == :sublist
    assert Sublist.compare([1], [2, 1]) == :sublist
  end

    # @tag :pending
  test "single element not exist in second list" do
    assert Sublist.compare([1], [2, 3]) == :unequal
  end

    # @tag :pending
  test "single element exist in the middle of second list" do
    assert Sublist.compare([1], [2, 1, 3]) == :sublist
  end

    # @tag :pending
  test "mulitple elements exists in the middle of second list" do
    assert Sublist.compare([1, 2], [3, 1, 2, 4]) == :sublist
    assert Sublist.compare([1, 2], [3, 1, 2]) == :sublist
  end
end

defmodule SuperlistTests do

  use ExUnit.Case

    # @tag :pending
  test "basic superlist" do
    assert Sublist.compare([1,2], [1]) == :superlist
    assert Sublist.compare([1,2], [2]) == :superlist
  end

    # @tag :pending
  test "list that is not a superlist" do
    assert Sublist.compare([1,2], [3]) == :unequal
  end

    # @tag :pending
  test "advanced superlist" do
    assert Sublist.compare([1, 2, 4, 2, 3, 4], [2, 3]) == :superlist
  end
end
